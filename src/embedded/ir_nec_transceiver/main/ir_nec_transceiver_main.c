...
#define COMMAND_1 0xF708
#define COMMAND_1_1 0xED12
#define COMMAND_2 0xEE11
#define COMMAND_3 0xF710
#define COMMAND_4 0xF711
...
static void relay_control(uint8_t pin, uint8_t state)
{
    /* Set the GPIO level according to the state (LOW or HIGH)*/
    ESP_LOGI(TAG,"\nSetting pin: %d to State:%d \n",pin,state);
...
static void example_parse_nec_frame(rmt_symbol_word_t *rmt_nec_symbols, size_t symbol_num)
{
    ...
    case 34: // NEC normal frame
        if (nec_parse_frame(rmt_nec_symbols))
        {
            ...
            if (s_nec_code_command == COMMAND_1)
            {
                relay_control(RELAY_1,1);
                printf("This is command 1\n");
            }
            else if(s_nec_code_command == COMMAND_1_1)
            {
                relay_control(RELAY_1,1);
                printf("This is command 1_1\n");
            }
            else if (s_nec_code_command == COMMAND_2)
            {
                printf("This is command 2\n");
                relay_control(RELAY_1,0);

            }
            else if (s_nec_code_command == COMMAND_3)
            {
                relay_control(RELAY_2,1);
                printf("This is command 3\n");
            }
            else if (s_nec_code_command == COMMAND_4)
            {
                relay_control(RELAY_2,0);
                printf("This is command 4\n");
            }
        }
        break;
